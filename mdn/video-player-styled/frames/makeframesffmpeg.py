...
for i in range(1,25):
	if i % 2 == 1:
		...
		if imgnext < 10:
			cv2.imwrite("img00"+str(imgnext)+".bmp", im)
			#sys.stdout.write(str(im.tostring()))
			#cv2_im = cv2.cvtColor(im,cv2.COLOR_BGR2RGB)
			#pil_im = Image.fromarray(cv2_im)
			f.write("file '"+"img00"+str(imgnext)+".jpeg"+"'\nduration 2\n")
			imgnext += 1
		elif imgnext < 100:
			cv2.imwrite("img0"+str(imgnext)+".bmp", im)
			#sys.stdout.write(str(im.tostring()))
			#cv2_im = cv2.cvtColor(im,cv2.COLOR_BGR2RGB)
			#pil_im = Image.fromarray(cv2_im)
			...
			cv2.imwrite("img"+str(imgnext)+".bmp", im)
			#sys.stdout.write(str(im.tostring()))
			#cv2_im = cv2.cvtColor(im,cv2.COLOR_BGR2RGB)
			#pil_im = Image.fromarray(cv2_im)
		...
		for ii in range(0,nmid+1):
			...
			if imgnext < 10:
				...
				cv2.imwrite("img0"+str(imgnext)+".bmp", im)
				#sys.stdout.buffer.write(im.tostring())
				#cv2_im = cv2.cvtColor(im,cv2.COLOR_BGR2RGB)
				#pil_im = Image.fromarray(cv2_im)
				if ii > 0:
					f.write("file '"+"img0"+str(imgnext)+".jpeg"+"'\nduration 0.05\n")
